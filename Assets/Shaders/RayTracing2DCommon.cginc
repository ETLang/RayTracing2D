...
RandomContext NextRandom(RandomContext ctx)
{
    ...
    ret.state.x = TausStep(ctx.state.x, 13, 19, 12, 4294967294);
    ret.state.y = TausStep(ctx.state.y, 2, 25, 4, 4294967288);
    ret.state.z = TausStep(ctx.state.z, 3, 11, 17, 4294967280);
    ret.state.w = LCGStep(ctx.state.w, 1664525, 1013904223);
    ret.value = 2.3283064365387e-10 * (ret.state.x ^ ret.state.y ^ ret.state.z ^ ret.state.w);
